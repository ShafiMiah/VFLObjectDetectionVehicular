...
class ActiveYOLOClient(NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        trainResult = YOLOModel.train(data='YOLOConfigClient'+str(self.client_number)+'.yaml', epochs = 2,imgsz=672,rect=True, device = 0 if torch.cuda.is_available() else 'cpu')
        ModelParameters.save_parameters(YOLOModel, self.parameter_path)
        print('TrainResult= MAP50: '+str(trainResult.box.map50)+ ' MAP95: '+ str(trainResult.box.map ))
...
def RunActiveClient(clientNumber):
    ...
    client = ActiveYOLOClient(train_sample, val_sample, clientNumber)
